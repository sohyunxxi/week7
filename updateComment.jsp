...
<%
    ...
    int commentIdx = Integer.parseInt(request.getParameter("cmIdx"));
    ...
    if(commentSet.next()){
        postWriterIdx = commentSet.getInt("user_idx");
        postIdx = commentSet.getInt("post_idx");
        commentIdx = commentSet.getInt("idx");
    }
    ...
    if(postWriterIdx==writerIdx){// 댓글 작성자와 로그인한 사람이 같은 사람이면 가능
        
        redirectPage = "changeComment.jsp?post_id=" + postIdx + "&comment_idx=" + commentIdx;

        message="댓글 수정 페이지로 이동합니다.";
    }
    else{ //다른 사람이면 불가능
        message="다른 사람의 댓글은 수정할 수 없습니다!";
