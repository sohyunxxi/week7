...
<%
    ...
    //session에 저장 idx 으로....ㅜㅜ

    String postSql = "SELECT idx,title,content FROM post WHERE user_idx=? ORDER BY idx DESC LIMIT 1";
...
<body>
    ...
    <main>
        ...
        <div id="mainText">
            <h1 name = "title" class="mainInfoText"> You can update your post! </h1>
            <p  class="mainInfoText">Update your post where you are going to fix...</p>
        </div>
        ...
        <div id="writeBox">
            <div id="titleBox">
                <span> Title </span> 
                <input name="updatedTitle" id="titleWriteBox" type="text" maxlength="30" length="32" value="<%=postTitle %>"> 
                
            </div>
            <div id="writeContentBox">
                <span id="contentTitle">Content</span>
                <textarea name="updatedContent"cols="105" rows="45"> <%=postContent %> </textarea>
            </div>
            <div id="buttonBox">
                <button class="mainCreatePostButton">수정완료</button></form>
                <form action ="deletePost.jsp">
                    <button class="mainCreatePostButton">삭제</button>
                    <input type="hidden" name="post_id" value="<%= postSet.getInt("idx") %>">
                </form>
